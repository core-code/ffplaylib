...
//#ifdef __APPLE__
//static const char *input_filename = "/Users/julian/Documents/Development/Core3D/CoreBreach/Resources/story/story_12.webm";
//#else
//static const char *input_filename = "/extradisk/Core3D/CoreBreach.app/Resources/story_1.webm";
//#endif
...
static int input_request_frame(AVFilterLink *link)
{
    ...
    if(priv->use_dr1 && priv->frame->opaque) {
        ...
        av_image_copy(picref->data, picref->linesize,
                     (const unsigned char **) priv->frame->data, priv->frame->linesize,
...
static int configure_video_filters(AVFilterGraph *graph, VideoState *is, const char *vfilters)
{
    ...
    if(vfilters) {
        ...
        {
            printf("avfilter_graph_parse failed");
            goto the_end;
        }
        av_freep(&vfilters);
    } else {
...
static int video_thread(void *arg)
{
    ...
    {
        printf("  video_thread configure_video_filters the_end %x %x %x %i\n", graph, is, vfilters, ret);
    ...
    for(;;) {
        ...
        if (ret < 0)
        {
            printf("  video_thread get_video_frame the_end\n");
